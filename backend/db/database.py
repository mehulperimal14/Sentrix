...
from core.paths import ALERTS_DIR

# Cross-platform SQLite path — stored inside backend/runtime/
_DB_DIR = Path(__file__).parent.parent / "runtime"
_DB_DIR.mkdir(parents=True, exist_ok=True)
DATABASE_URL = f"sqlite:///{_DB_DIR / 'sentrix.db'}"
...
def prune_old_snapshots(days: int = None):
    ...
    deleted = 0
    try:
        for fpath in ALERTS_DIR.iterdir():
            if not fpath.suffix.lower() in (".jpg", ".jpeg", ".png"):
                continue  # Only delete snapshot images, never .enc/.json
            try:
                if fpath.is_file() and fpath.stat().st_mtime < cutoff_ts:
                    fpath.unlink()
                    deleted += 1
            except Exception:
                continue
