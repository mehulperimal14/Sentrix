...
templates = Jinja2Templates(
    directory=str(Path(__file__).parent.parent / "runtime" / "templates")
)

from core.paths import ALERTS_DIR, AUTH_FACES_DIR, ALERTS_DIR_STR, AUTH_FACES_DIR_STR
AUTHORIZED_DIR = AUTH_FACES_DIR_STR
ALERTS_DIR_STR_ROUTE = ALERTS_DIR_STR
...
async def alerts_view(request: Request):
    ...
    if ALERTS_DIR.is_dir():
        for fname in sorted(ALERTS_DIR.iterdir(), reverse=True):
            if fname.suffix.lower() in (".jpg", ".jpeg", ".png"):
                images.append(f"/static/alerts/{fname.name}")
...
async def authorized_view(request: Request):
    faces = []
    if AUTH_FACES_DIR.is_dir():
        for fname in AUTH_FACES_DIR.iterdir():
            if fname.suffix.lower() in (".jpg", ".jpeg", ".png"):
                faces.append({"filename": fname.name, "url": f"/static/authorized_faces/{fname.name}"})
...
async def upload_face(file: UploadFile = File(...)):
    ...
    try:
        ...
        dest = str(AUTH_FACES_DIR / safe_filename)
