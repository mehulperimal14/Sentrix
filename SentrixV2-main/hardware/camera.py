...
RECONNECT_COOLDOWN = 6.0           # Seconds between device reconnect attempts
...
class Camera:
    ...
    def _try_open_source(self) -> bool:
        """Attempt to open camera device cleanly."""
        if self.cap:
            try:
                self.cap.release()
            except Exception:
                ...
                try:
                    ...
                    if cap and cap.isOpened():
                        ...
                        if ret and frame is not None and frame.size > 0:
                            self.cap = cap
                            print(f"[Camera] Connected to camera index {self.source} ({frame.shape[1]}x{frame.shape[0]})")
                            return True
                        cap.release()
                except Exception:
                    continue
