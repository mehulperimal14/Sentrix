# training/scripts/train_anomaly_classifier.py
#
# High-Accuracy Anomaly Classifier using PyTorch ResNet18 Transfer Learning.
# Output saved to backend/models/anomaly_classifier.pt
...
OUTPUT_MODEL = BACKEND_MODELS_DIR / "anomaly_classifier.pt"
...
class AnomalyDataset(Dataset):
    def __init__(self, csv_file, data_dir, transform=None):
        ...
        for _, row in df.iterrows():
            ...
            label = 1 if "anom" in label_str else 0
            
            fname = Path(orig_path).name
            subfolder = "anomalous" if label == 1 else "normal"
...
def train(epochs=5, batch_size=32, lr=3e-4):
    print("=" * 60)
    print("SENTRIX: High-Accuracy Anomaly Classifier (ResNet18)")
    ...
    train_ds = AnomalyDataset(DATA_DIR / "train.csv", DATA_DIR, transform=train_tf)
    val_ds = AnomalyDataset(DATA_DIR / "val.csv", DATA_DIR, transform=val_tf)
    ...
    print(f"\n✅ Anomaly classifier training complete. Final model: {OUTPUT_MODEL}")
